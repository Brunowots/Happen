...
int NpcFunctions::luaNpcSellItem(lua_State* L) {
	...
	if (inBackpacks) {
		...
		while (remainingAmount > 0) {
			...
			if (player->purchaseBuyIntoContainer()) {
				ret = g_game().internalQuickLootItem(player, item, g_game().getObjectCategory(item));
			}

			if (ret != RETURNVALUE_NOERROR || !player->purchaseBuyIntoContainer()){
				ret = g_game().internalPlayerAddItem(player, item, ignoreCap, CONST_SLOT_WHEREEVER);
			}
