...
void Container::setBodyFresh(bool value)
{
	// Only update when necessary
	if (freshBody != value) {
		freshBody = value;
		SpectatorHashSet spectators;
		g_game().map.getSpectators(spectators, this->getPosition(), true);
		for (Creature* spectator : spectators) {
			if (Player* player = spectator->getPlayer()) {
				player->sendUpdateTileItem(this->getTile(), this->getPosition(), this);
			}
		}
		return;
	}

	// Silently update the identifier
	freshBody = value;
}
