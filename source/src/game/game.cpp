...
void Game::playerSetBuyToContainer(uint32_t playerid, bool enabled)
{
	Player* player = getPlayerByID(playerid);
	if (!player) {
		return;
	...
	player->setPurchaseBuyIntoContainer(enabled);
}
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage &damage, bool isEvent /*= false*/) {
	...
	if (damage.primary.value > 0) {
		...
		if (attackerPlayer) {
			// Item spell aguments
			double itemModifier = attackerPlayer->getSpellImpactModifier(damage.instantSpellName);
			damage.primary.value = damage.primary.value * itemModifier;
			damage.secondary.value = damage.secondary.value * itemModifier;
		}
...
void Game::applyManaLeech(
	...
	uint16_t manaChance = attackerPlayer->getSkillLevel(SKILL_MANA_LEECH_CHANCE) + attackerPlayer->getSpellSkillModifier(damage.instantSpellName, SKILL_MANA_LEECH_CHANCE);
	uint16_t manaSkill = attackerPlayer->getSkillLevel(SKILL_MANA_LEECH_AMOUNT) + attackerPlayer->getSpellSkillModifier(damage.instantSpellName, SKILL_MANA_LEECH_AMOUNT);
	if (normal_random(0, 100) >= manaChance) {
		return;
	}
	// Void charm rune
...
void Game::applyLifeLeech(
	...
	uint16_t lifeChance = attackerPlayer->getSkillLevel(SKILL_LIFE_LEECH_CHANCE) + attackerPlayer->getSpellSkillModifier(damage.instantSpellName, SKILL_LIFE_LEECH_CHANCE);
	uint16_t lifeSkill = attackerPlayer->getSkillLevel(SKILL_LIFE_LEECH_AMOUNT) + attackerPlayer->getSpellSkillModifier(damage.instantSpellName, SKILL_LIFE_LEECH_AMOUNT);
	if (normal_random(0, 100) >= lifeChance) {
		return;
	}
	if (targetMonster) {
