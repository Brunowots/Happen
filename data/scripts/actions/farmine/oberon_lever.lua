...
function oberonLever.onUse(creature, item, fromPosition, target, toPosition, isHotkey)
		if item.itemid == 2772 and item.actionid == 57605 then
			 local player = creature:isPlayer() and Player(creature) or nil
  		if not player or player:getLevel() < 250 then
  		  player:sendCancelMessage("You need to be at least level 250 to use this lever.")
		    return true
  		end
