...
class Player final : public Creature, public Cylinder {
	public:
		...
		double getSpellImpactModifier(std::string name) const;
		int32_t getSpellSkillModifier(std::string name, skills_t skill) const;
		int32_t getSpellCooldownModifier(std::string name) const;
		
		const std::vector<uint32_t>& getBosstiaryTrackerList() const {
			return bosstiaryTracker;
		}
		bool isBossOnBosstiaryTracker(uint16_t bossid) const;
		void addBossOnBosstiaryTracker(uint16_t bossid, bool refreshUi = true);
		void refreshBosstiaryTracker(const std::vector<uint32_t>& list) {
			if (client) {
				client->refreshBosstiaryTracker(list);
			}
		}
		...
		bool purchaseBuyIntoContainer() const {
			return purchaseBuyToContainer;
		}
		void setPurchaseBuyIntoContainer(bool enabled) {
			purchaseBuyToContainer = enabled;
		}
		...
		int32_t getWeaponSkill(const Item* item, std::string instantSpellName = "") const;
	...
	private:
		...
		std::vector<uint32_t> bosstiaryTracker;
		...
		bool moved = false;
		bool dead = false;
		bool purchaseBuyToContainer = false;
