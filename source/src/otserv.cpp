...
void loadModules() {
	...
	SPDLOG_INFO("Server protocol: {}.{}{}", CLIENT_VERSION_UPPER, CLIENT_VERSION_LOWER, g_configManager().getBoolean(OLD_PROTOCOL) ? " and 10X,OTC, ANDROID CLIENTE!" : "");
	...
	if (!useAnyDatapack && (datapackName != "data" && datapackName != "data" || datapackName != "data" && datapackName != "data")) {
		startupErrorMessage();
	}
	...
	if (!DatabaseManager::isDatabaseSetup()) {
		SPDLOG_ERROR("The database you have specified in {} is empty, " "please import the schema.sql to your database.", g_configManager().getConfigFileLua());
		startupErrorMessage();
	}
	...
	SPDLOG_INFO("Server Private By Oliveira DEV: Version 13.20");
...
int main(int argc, char* argv[]) {
	#ifdef DEBUG_LOG
	SPDLOG_DEBUG("[HAPPEN] SPDLOG LOG DEBUG ENABLED");
...
void mainLoader(int, char*[], ServiceManager* services) {
	...
	if (getuid() == 0 || geteuid() == 0) {
		SPDLOG_INFO("{} HappenOT Custom Server Private Online Successfully", STATUS_SERVER_NAME);
	}
#endif
	...
	webhook_send_message("HappenOT Custom is now online", "HappenOT Custom successfully started.", WEBHOOK_COLOR_ONLINE, url);
