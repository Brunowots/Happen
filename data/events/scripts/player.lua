...
function Player:onLook(thing, position, distance)
	local description = "You see " description = onItemSetLook(self, thing, position, distance, description)
...
function Player:onMoveItem(item, count, fromPosition, toPosition, fromCylinder, toCylinder)
	...
	-- Lixeira da roleta
	local trash = Tile(toPosition)
	if trash then
		local ground = trash:getGround()
		if ground and ground:getActionId() == 2154 then
			if item:remove() then
				local effect = Position(toPosition)
				doSendMagicEffect(effect, CONST_ME_HITBYFIRE)
				return false
			end
		end
	end
...
function Player:onTradeRequest(target, item)
	local protectedArea = {
		fromThais = Position(32360, 32251, 7),
		toThais = Position(32374, 32272, 7),

		fromThais2 = Position(32360, 32251, 7),
		toThais2 = Position(32374, 32272, 7),

	}
	local position
    position = self:getPosition()
    if position and position:isInRange(protectedArea.fromThais, protectedArea.toThais) then
        self:sendTextMessage(MESSAGE_INFO_DESCR, "You cannot trade here.")
        return false
	elseif position and position:isInRange(protectedArea.fromThais2, protectedArea.toThais2) then
        self:sendTextMessage(MESSAGE_INFO_DESCR, "You cannot trade here.")
        return false
    end
...
local baseRate = self:getFinalBaseRateExperience()
    local baseExperience = (exp * baseRate + (exp * (storeXpBoostAmount/100))) * staminaBoost
    if self:getStorageValue(3265988) > 0 then
        local porcentagem = 20
        local aumento = porcentagem / 100 + 1
        baseExperience = baseExperience * aumento 
    end
