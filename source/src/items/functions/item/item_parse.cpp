...
void ItemParse::initParse(const std::string &tmpStrValue, pugi::xml_node attributeNode, pugi::xml_attribute valueAttribute, ItemType &itemType) {
	...
	ItemParse::parseAugments(tmpStrValue, attributeNode, valueAttribute, itemType);
...
void ItemParse::parseAugments(const std::string &tmpStrValue, pugi::xml_node attributeNode, pugi::xml_attribute valueAttribute, ItemType &itemType) {
	if (tmpStrValue != "spellboost") {
		return;
	}

	std::string spellValue = asLowerCaseString(valueAttribute.as_string());
	if (spellValue == "impact") {
		for (auto subAttributeNode : attributeNode.children()) {
			pugi::xml_attribute spellNameAttribute = subAttributeNode.attribute("name");
			if (!spellNameAttribute) {
				continue;
			}

			pugi::xml_attribute spellValueAttribute = subAttributeNode.attribute("value");
			if (!spellValueAttribute) {
				continue;
			}

			pugi::xml_attribute spellAppendAttribute = subAttributeNode.attribute("append");
			if (!spellAppendAttribute) {
				continue;
			}

			ItemAugmentModifier itemAugmentModifier;
			itemAugmentModifier.name = spellNameAttribute.as_string();
			itemAugmentModifier.append = spellAppendAttribute.as_string();
			itemAugmentModifier.impact = pugi::cast<double>(spellValueAttribute.value());
			itemType.spellBoosts[asLowerCaseString(itemAugmentModifier.name)].push_back(itemAugmentModifier);
		}
	} else if (spellValue == "cooldown") {
		for (auto subAttributeNode : attributeNode.children()) {
			pugi::xml_attribute spellNameAttribute = subAttributeNode.attribute("name");
			if (!spellNameAttribute) {
				continue;
			...
			ss << spellValueAttribute.as_string() << "s " << spellAppendAttribute.as_string();
			ItemAugmentModifier itemAugmentModifier;
			itemAugmentModifier.name = spellNameAttribute.as_string();
			itemAugmentModifier.append = ss.str();
			itemAugmentModifier.cooldown = pugi::cast<int32_t>(spellValueAttribute.value());
			itemType.spellBoosts[asLowerCaseString(itemAugmentModifier.name)].push_back(itemAugmentModifier);
		}
	} else if (spellValue == "skill") {
		for (auto subAttributeNode : attributeNode.children()) {
			pugi::xml_attribute nameAttribute = subAttributeNode.attribute("name");
			if (!nameAttribute) {
				continue;
			}

			pugi::xml_attribute skillAttribute = subAttributeNode.attribute("skill");
			if (!skillAttribute) {
				continue;
			}

			pugi::xml_attribute valueAttribute = subAttributeNode.attribute("value");
			if (!valueAttribute) {
				continue;
			}

			pugi::xml_attribute appendAttribute = subAttributeNode.attribute("append");
			if (!appendAttribute) {
				continue;
			}

			ItemAugmentModifier itemAugmentModifier;
			itemAugmentModifier.name = nameAttribute.as_string();

			int32_t percentage = 0;
			std::string skillValue = skillAttribute.as_string();
			if (skillValue == "skillsword") {
				itemAugmentModifier.skills[SKILL_SWORD] = pugi::cast<int32_t>(valueAttribute.value());
			} else if (skillValue == "skillaxe") {
				itemAugmentModifier.skills[SKILL_AXE] = pugi::cast<int32_t>(valueAttribute.value());
			} else if (skillValue == "skillclub") {
				itemAugmentModifier.skills[SKILL_CLUB] = pugi::cast<int32_t>(valueAttribute.value());
			} else if (skillValue == "skilldist") {
				itemAugmentModifier.skills[SKILL_DISTANCE] = pugi::cast<int32_t>(valueAttribute.value());
			} else if (skillValue == "skillfish") {
				itemAugmentModifier.skills[SKILL_FISHING] = pugi::cast<int32_t>(valueAttribute.value());
			} else if (skillValue == "skillshield") {
				itemAugmentModifier.skills[SKILL_SHIELD] = pugi::cast<int32_t>(valueAttribute.value());
			} else if (skillValue == "skillfist") {
				itemAugmentModifier.skills[SKILL_FIST] = pugi::cast<int32_t>(valueAttribute.value());
			} else if (skillValue == "skillcriticalchance") {
				itemAugmentModifier.skills[SKILL_CRITICAL_HIT_CHANCE] = pugi::cast<int32_t>(valueAttribute.value());
			} else if (skillValue == "skillcriticaldamage") {
				itemAugmentModifier.skills[SKILL_CRITICAL_HIT_DAMAGE] = pugi::cast<int32_t>(valueAttribute.value());
			} else if (skillValue == "skilllifechance") {
				itemAugmentModifier.skills[SKILL_LIFE_LEECH_CHANCE] = pugi::cast<int32_t>(valueAttribute.value());
			} else if (skillValue == "skilllifeamount") {
				itemAugmentModifier.skills[SKILL_LIFE_LEECH_AMOUNT] = pugi::cast<int32_t>(valueAttribute.value());
			} else if (skillValue == "skillmanachance") {
				itemAugmentModifier.skills[SKILL_MANA_LEECH_CHANCE] = pugi::cast<int32_t>(valueAttribute.value());
			} else if (skillValue == "skillmanaamount") {
				itemAugmentModifier.skills[SKILL_MANA_LEECH_AMOUNT] = pugi::cast<int32_t>(valueAttribute.value());
			} else {
				SPDLOG_WARN("[ParseAugments::parseSkill] - Unknown skill type: {}", skillValue);
				continue;
			}
			std::ostringstream ss;
			ss << valueAttribute.as_string() << "% " << appendAttribute.as_string();
			itemAugmentModifier.append = ss.str();
			itemType.spellBoosts[asLowerCaseString(itemAugmentModifier.name)].push_back(itemAugmentModifier);
		}
	}
