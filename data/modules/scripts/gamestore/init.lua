...
function convertType(type)
	local types = {
		...
		[GameStore.OfferTypes.OFFER_TYPE_HOUSE_BED] = GameStore.ConverType.SHOW_NONE,
	...
	for name, offer in pairs(offers) do
		if count > 0 then
			...
			if player:getClient().version > 1200 then
				if isInArray(runesMwColor, offer.itemtype) then
					for i = 1, #runesMwColor do
						if runesMwColor[i] == offer.itemtype then
							msg:addU16(wallsMwColor[i])
						end
					end
				else
...
function GameStore.processPremiumPurchase(player, offerId)
	...
	player:setLoaltyPointsDb(((offerId - 3000)/30))
end
...
function GameStore.processHouseBedPurchase(player, offerId, offerCount, offerName)
	...
	local inbox = player:getSlotItem(CONST_SLOT_STORE_INBOX)
	if inbox and inbox:getEmptySlots() > 0 then
		local parcel = inbox:addItem(3503, 1)
		if parcel then
			local bedKit = parcel:addItem(23398, 1)
			if bedKit then
			...
			local bedPartKit = parcel:addItem(23398, 1)
			if bedPartKit then
				...
				bedPartKit:setCustomAttribute("unWrapId", (offerId+1))
			end
		end
