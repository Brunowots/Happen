...
void ProtocolGame::AddItem(NetworkMessage &msg, const Item* item) {
	...
	if (it.isContainer()) {
		...
		// Quiver ammo count
		if (containerType == 0 && container && item->isQuiver() && player->getThing(CONST_SLOT_RIGHT) == item) {
		...
		// Corpse with loot
		if (containerType == 0) {
			if (container && (container->getCorpseOwner() == static_cast<uint32_t>(std::numeric_limits<int32_t>::max()) || (container->isFresh() && (container->getCorpseOwner() == 0 || player->canOpenCorpse(container->getCorpseOwner()))))) {
				containerType = 4;
				msg.addByte(containerType);
			}
		}
...
void ProtocolGame::parseBuyToContainer(NetworkMessage& msg)
{
	bool enabled = msg.getByte() != 0;
	addGameTask(&Game::playerSetBuyToContainer, player->getID(), enabled);
}
...
void ProtocolGame::parseBestiarysendMonsterData(NetworkMessage &msg) {
	...
	if (currentLevel > 1) {
		...
		newmsg.addDouble(mtype->info.mitigation); // 13.00 Mitigation
	}
...
void ProtocolGame::addBestiaryTrackerList(NetworkMessage &msg) {
	...
	if (g_ioBosstiary().getMonsterTypeByBossRaceId(static_cast<uint32_t>(thisrace)) != nullptr) {
		player->addBossOnBosstiaryTracker(static_cast<uint32_t>(thisrace));
		return;
	}
	std::map<uint16_t, std::string> mtype_list = g_game().getBestiaryList();
...
void ProtocolGame::refreshBosstiaryTracker(const std::vector<uint32_t>& list)
{
	if (!player || oldProtocol || list.empty()) {
		return;
	}
	...
	for (const auto& bossid : list) {
		MonsterType* mtype = g_ioBosstiary().getMonsterTypeByBossRaceId(bossid);
		if (!mtype) {
			return;
		}
		...
		msg.add<uint16_t>(bossid);
		msg.add<uint32_t>(player->getBestiaryKillCount(static_cast<uint16_t>(bossid)));
		for (const auto& stage : stages) {
			msg.add<uint16_t>(stage.kills);
		}

		if (g_ioBosstiary().getBossCurrentLevel(player, bossid) == 3) {
			msg.addByte(4);
		} else {
			msg.addByte(0);
		}
...
void ProtocolGame::sendCreatureIcon(const Creature* creature) {
	...
	int size = 0;
	auto start = msg.getBufferPosition();
	msg.skipBytes(1);
	...
	if (version > 1289 && creature->getPlayer() && creature->getPlayer()->getHazardSystemReference() > 0 && creature->getPlayer()->getHazardSystemPoints() > 0) {
		msg.addByte(22); // Hazard icon
		msg.addByte(0);
		msg.add<uint16_t>(creature->getPlayer()->getHazardSystemPoints());
		size++;
	}
	msg.setBufferPosition(start);
	msg.addByte(size);
	writeToOutputBuffer(msg);
}
...
void ProtocolGame::sendMarketDetail(uint16_t itemId, uint8_t tier) {
	...
	if (!oldProtocol) {
		// Augments
		if (it.spellBoosts.empty()) {
			msg.add<uint16_t>(0x00);
		} else {
			std::ostringstream ss;
			bool addComma = false;
			for (const auto& spellInfo : it.spellBoosts) {
				for (const auto& spellBoost : spellInfo.second) {
					if (addComma) {
						ss << ", ";
					}
					addComma = true;
					ss << spellBoost.name << " -> " << spellBoost.append;
				}
			}
			msg.addString(ss.str());
		}
...
void ProtocolGame::sendOutfitWindow() {
	...
	if (oldProtocol) {
		...
		for (const Outfit &outfit : outfits) {
			...
			// Game client doesn't allow more than 150 outfits
	...
	auto startOutfits = msg.getBufferPosition();
	// 150 is the limit of old protocol clients.
...
void ProtocolGame::AddCreature(NetworkMessage &msg, const Creature* creature, bool known, uint32_t remove) {
	...
	if (!oldProtocol) {
		if (otherPlayer) {
			...
			auto end = msg.getBufferPosition();
			msg.setBufferPosition(start);
			msg.addByte(size);
			msg.setBufferPosition(end);
			} else {
...
void ProtocolGame::parseSendBosstiary() {
	...
	for (const auto &[bossid, name] : mtype_map) {
		...
		auto bossRace = static_cast<uint8_t>(magic_enum::enum_integer<BosstiaryRarity_t>(mType->info.bosstiaryRace));
		if (bossRace == 0xFF) {
			SPDLOG_ERROR("[ProtocolGame::parseSendBosstiary] - Falha na janela bosstiary, boss com nome '{}' possui uma race invalida e foi ignorado.", name);
			continue;
		}
		...
		msg.addByte(0);
		msg.addByte(player->isBossOnBosstiaryTracker(bossid) ? 0x01 : 0x00);
		++bossesCount;
	}
