...
int32_t Player::getWeaponSkill(const Item* item, std::string instantSpellName/* = ""*/) const {
	if (!item) {
		return getSkillLevel(SKILL_FIST) + getSpellSkillModifier(instantSpellName, SKILL_FIST);
	}
	...
	switch (weaponType) {
		case WEAPON_SWORD: {
			attackSkill = getSkillLevel(SKILL_SWORD) + getSpellSkillModifier(instantSpellName, SKILL_SWORD);
			break;
		}

		case WEAPON_CLUB: {
			attackSkill = getSkillLevel(SKILL_CLUB) + getSpellSkillModifier(instantSpellName, SKILL_CLUB);
			break;
		}

		case WEAPON_AXE: {
			attackSkill = getSkillLevel(SKILL_AXE) + getSpellSkillModifier(instantSpellName, SKILL_AXE);
			break;
		}

		case WEAPON_MISSILE:
		case WEAPON_DISTANCE: {
			attackSkill = getSkillLevel(SKILL_DISTANCE) + getSpellSkillModifier(instantSpellName, SKILL_DISTANCE);
			break;
		}
...
uint16_t Player::getLoyaltySkill(skills_t skill) const {
	...
	if (vocation->getId() == VOCATION_KNIGHT || vocation->getId() == VOCATION_ELITE_KNIGHT) {
		if (skill != SKILL_DISTANCE) {
			level = level + getLoyaltyBonus();
		}
	} 
	if (vocation->getId() == VOCATION_PALADIN || vocation->getId() == VOCATION_ROYAL_PALADIN) {
		if (skill == SKILL_DISTANCE || skill == SKILL_FIST || skill == SKILL_SHIELD || skill == SKILL_FISHING) {
			level = level + getLoyaltyBonus();
		}
	} 
	if (vocation->getId() == VOCATION_DRUID || vocation->getId() == VOCATION_SORCERER || vocation->getId() == VOCATION_MASTER_SORCERER || vocation->getId() == VOCATION_ELDER_DRUID) {
		if (skill == SKILL_FIST || skill == SKILL_SHIELD || skill == SKILL_FISHING) {
			level = level + getLoyaltyBonus();
		}
	}
...
double Player::getSpellImpactModifier(std::string name) const
{
	double modifier = 1.0;
	for (uint32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_LAST; slot++) {
		...
		modifier += item->getSpellImpactModifier(name) - 1.0;
	}

	return modifier;
}

int32_t Player::getSpellSkillModifier(std::string name, skills_t skill) const
{
	if (name == "") {
		return 0;
	}
	...
	for (uint32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_LAST; slot++) {
		...
		modifier += item->getSpellSkillModifier(name, skill);
	}

	return modifier;
}

int32_t Player::getSpellCooldownModifier(std::string name) const
{
	int32_t modifier = 0;
	for (uint32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_LAST; slot++) {
		Item* item = inventory[slot];
		...
		modifier += item->getSpellCooldownModifier(name);
	}
...
bool Player::isBossOnBosstiaryTracker(uint16_t bossid) const
{
	auto it = std::find(bosstiaryTracker.begin(), bosstiaryTracker.end(), bossid);
	if (it == bosstiaryTracker.end()) {
		return false;
	}
...
void Player::addBossOnBosstiaryTracker(uint16_t bossid, bool refreshUi/* = true*/)
{
	auto it = std::find(bosstiaryTracker.begin(), bosstiaryTracker.end(), bossid);
	if (it != bosstiaryTracker.end()) {
		bosstiaryTracker.erase(it);
		if (refreshUi && client) {
			client->parseSendBosstiary();
			client->refreshBosstiaryTracker(getBosstiaryTrackerList());
		}
		return;
	}
	
	if (getBestiaryKillCount(bossid) <= 0) {
		return;
	}

	bosstiaryTracker.push_back(bossid);
	if (refreshUi && client) {
		client->parseSendBosstiary();
		client->refreshBosstiaryTracker(getBosstiaryTrackerList());
	}
