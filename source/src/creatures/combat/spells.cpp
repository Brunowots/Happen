...
void Spell::applyCooldownConditions(Player* player) const {
	...
	if (cooldown > 0) {
		...
		int32_t currentCooldown = static_cast<int32_t>(cooldown) + player->getSpellCooldownModifier(getName());
	if (currentCooldown > 0) {
		Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_SPELLCOOLDOWN, currentCooldown / g_configManager().getFloat(RATE_SPELL_COOLDOWN), 0, false, spellId);
