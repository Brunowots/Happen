...
void Combat::doCombatHealth(Creature* caster, Creature* target, CombatDamage &damage, const CombatParams &params) {
	...
	if (caster && caster->getPlayer()) {
		// Critical damage
		uint16_t chance = caster->getPlayer()->getSkillLevel(SKILL_CRITICAL_HIT_CHANCE) + damage.criticalChance + caster->getPlayer()->getSpellSkillModifier(damage.instantSpellName, SKILL_CRITICAL_HIT_CHANCE);
		// Charm low blow rune)
		...
		if (chance != 0 && uniform_random(1, 100) <= chance) {
			...
			damage.primary.value += (damage.primary.value * (caster->getPlayer()->getSkillLevel(SKILL_CRITICAL_HIT_DAMAGE) + damage.criticalDamage) + caster->getPlayer()->getSpellSkillModifier(damage.instantSpellName, SKILL_CRITICAL_HIT_DAMAGE)) / 100;
			damage.secondary.value += (damage.secondary.value * (caster->getPlayer()->getSkillLevel(SKILL_CRITICAL_HIT_DAMAGE) + damage.criticalDamage) + caster->getPlayer()->getSpellSkillModifier(damage.instantSpellName, SKILL_CRITICAL_HIT_DAMAGE)) / 100;
		}
...
void ValueCallback::getMinMaxValues(Player* player, CombatDamage &damage, bool useCharges) const {
	...
	switch (type) {
		...
		case COMBAT_FORMULA_SKILL: {
			...
			lua_pushnumber(L, player->getWeaponSkill(item ? item : tool, damage.instantSpellName));
