...
double ItemType::getSpellImpactModifier(std::string name) const
{
	double modifier = 1.0;
	for (const auto& spellInfo : spellBoosts) {
		if (spellInfo.first == asLowerCaseString(name)) {
			for (const auto& spellBoost : spellInfo.second) {
				modifier += spellBoost.impact - 1.0;
			}
		}
	}
...
int32_t ItemType::getSpellSkillModifier(std::string name, skills_t skill) const
{
	int32_t modifier = 0;
	for (const auto& spellInfo : spellBoosts) {
		if (spellInfo.first == asLowerCaseString(name)) {
			for (const auto& spellBoost : spellInfo.second) {
				modifier += spellBoost.skills[skill];
			}
		}
	}
...
int32_t ItemType::getSpellCooldownModifier(std::string name) const
{
	int32_t modifier = 0;
	for (const auto& spellInfo : spellBoosts) {
		if (spellInfo.first == asLowerCaseString(name)) {
			for (const auto& spellBoost : spellInfo.second) {
				modifier += spellBoost.cooldown;
			}
		}
	}
