...
function citizen.onStepIn(creature, item, position, fromPosition)
	...
	if town:getId() == TOWNS_LIST.SVARGROND and player:getStorageValue(Storage.BarbarianTest.Questline) < 8 then
		player:sendTextMessage(MESSAGE_GAME_HIGHLIGHT, 'Você primeiro precisa absolver a Missão de Teste Bárbaro para se tornar cidadão!')
	...
	player:teleportTo(town:getTemplePosition())
	player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'Agora você é um cidadão HappenOT.')
	return true
end
