dofile(CORE_DIRECTORY.. "/scripts/reward_chest/items_rares.lua")
		...
		for _, con in ipairs(scores) do
			...
			if con.score ~= 0 then
				...
				for _, p in ipairs(playerLoot) do
					...
					if isInArray(items_rares_Site, p[1], true) then
						local itemtype = ItemType(p[1])
						db.query("UPDATE `players` SET `drops` = `drops` + 1 WHERE `id` = " .. player:getGuid())
						db.query("INSERT INTO `rares`(`item`, `playerName`, `boss`, `date`) VALUES ("..p[1]..", '"..player:getName().."', '"..monsterType:getName().."', "..os.time()..")")
						db.query('INSERT INTO `rares_items`(`itemid`, `drops`, `itemName`) VALUES ('..p[1]..', 1, "'..itemtype:getName()..'") ON DUPLICATE KEY UPDATE `drops` = `drops` + 1')
					end
				end
