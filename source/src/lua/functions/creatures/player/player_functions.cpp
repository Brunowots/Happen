...
int PlayerFunctions::luaPlayerResetSpellsCooldown(lua_State* L) {
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
...
int PlayerFunctions::luaPlayerGetSpellImpactModifier(lua_State* L)
{
	// player:getSpellImpactModifier(spell)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	lua_pushnumber(L, player->getSpellImpactModifier(getString(L, 2)));
 	return 1;
}

int PlayerFunctions::luaPlayerGetSpellSkillModifier(lua_State* L)
{
	// player:getSpellSkillModifier(spell, skill)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	lua_pushnumber(L, player->getSpellSkillModifier(getString(L, 2), getNumber<skills_t>(L, 3)));
 	return 1;
}

int PlayerFunctions::luaPlayerGetSpellCooldownModifier(lua_State* L)
{
	// player:getSpellCooldownModifier(spell)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	lua_pushnumber(L, player->getSpellCooldownModifier(getString(L, 2)));
 	return 1;
}
